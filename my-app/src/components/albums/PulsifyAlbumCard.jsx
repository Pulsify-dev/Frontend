...
export const PulsifyAlbumCard = ({ album, onDelete }) => {
  ...
  const artistName =
    localAlbum.artist_id?.display_name || localAlbum.artist_name || "You";
  const [menuOpen, setMenuOpen] = useState(false);
    ...
    if (localAlbum) {
      if (localAlbum.is_liked !== undefined) {
        ...
        PulsifyAlbumService.checkIfLiked(albId)
          .then((res) => setIsLiked(res.liked || res.is_liked || false))
          .catch((err) =>
            console.error("Failed to check album like status", err),
          );
      }
    }
  }, [localAlbum, albId]);
  ...
  return (
    <div className="pulsify-playlist-card">
      <div
        ...
        onClick={() => navigate(`/albums/${albId}`)}
        style={{ cursor: "pointer" }}
      >
        <img
          src={
            localAlbum.artwork_url ||
        ...
        <div
          ...
          onClick={(e) => {
            ...
            if (localAlbum.tracks && localAlbum.tracks.length > 0) {
              const rawTrack =
                localAlbum.tracks[0].track_id &&
                typeof localAlbum.tracks[0].track_id === "object"
                  ? localAlbum.tracks[0].track_id
                  : localAlbum.tracks[0];
              const firstTrack =
                rawTrack._id && !rawTrack.id
                  ? { ...rawTrack, id: rawTrack._id }
                  : rawTrack;
              activeAlbumId = albId;
        ...
        >
          ...
          activeAlbumId === albId &&
          localAlbum.tracks?.[0] &&
          (localAlbum.tracks[0].track_id?._id ||
            localAlbum.tracks[0]._id ||
            localAlbum.tracks[0].id) === (currentTrack._id || currentTrack.id)
            ? "⏸"
            : "▶"}
        </div>
      </div>
      ...
      <div className="pulsify-card-body">
        <div className="pulsify-card-header-row">
          <button
            ...
            title={
              isPlaying && currentTrack && activeAlbumId === albId
                ? "Pause"
                : "Play"
            }
            onClick={(e) => {
          ...
          <div className="pulsify-card-info">
            ...
            <Link to={`/albums/${albId}`} className="pulsify-card-name">
              {localAlbum.title}
            </Link>
          </div>
          ...
          >
            <span className={`pulsify-album-type-badge type-${albumTypeLower}`}>
              {localAlbum.type || "Album"}
            </span>
        ...
        {localAlbum.tracks && localAlbum.tracks.length > 0 && (
          <div className="pulsify-card-tracklist">
            {localAlbum.tracks.slice(0, 5).map((t, i) => {
              ...
              const trackData =
                rawTrackData._id && !rawTrackData.id
                  ? { ...rawTrackData, id: rawTrackData._id }
                  : rawTrackData;
              return (
                <AlbumCardTrackEntry
