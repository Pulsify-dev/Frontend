    ...
    tracks: [
      {
        track_id: {
          ...
          artwork_url: "",
          duration: 215,
          artist_id: { _id: "a1", display_name: "ChillHop" },
        },
        position: 0,
      ...
      },
      {
        track_id: {
          ...
          artwork_url: "",
          duration: 198,
          artist_id: { _id: "a2", display_name: "LofiGirl" },
        },
        position: 1,
      ...
      },
      {
        track_id: {
          _id: "trk-3",
          title: "cafe study mix",
          genre: "Lo-Fi",
          artwork_url: "",
          duration: 221,
          artist_id: { _id: "a3", display_name: "JazzCat" },
        },
        position: 2,
    ...
    tracks: [
      {
        track_id: {
          _id: "trk-4",
          title: "Hardstyle Track A",
          genre: "Hardstyle",
          artwork_url: "",
          duration: 240,
          artist_id: { _id: "a4", display_name: "ZYZZ" },
        },
        position: 0,
      ...
      },
      {
        track_id: {
          _id: "trk-5",
          title: "Hardstyle Track B",
          genre: "Hardstyle",
          artwork_url: "",
          duration: 240,
          artist_id: { _id: "a5", display_name: "TEVVEZ" },
        },
        position: 1,
        added_at: "2026-04-12T08:01:00Z",
        _id: "te5",
      },
    ],
...
export const PulsifyPlaylistService = {
  ...
  async getMyPlaylists(page = 1, limit = 20) {
    if (isMock()) {
      return {
        ...
        data: mockPlaylists.map((p) => ({ ...p })),
      };
    }
    const { data } = await pulsifyAxiosInstance.get("/playlists", {
      params: { page, limit },
    });
    return data;
  },
  ...
  async getUserPlaylists(userId, page = 1, limit = 20) {
    ...
    const { data } = await pulsifyAxiosInstance.get(
  ...
  async getPlaylistById(playlistId, secretToken = null) {
    if (isMock()) {
      const found =
        mockPlaylists.find((p) => p._id === playlistId) || mockPlaylists[0];
  ...
  async getByPermalink(permalink, secretToken = null) {
    if (isMock()) {
      const found =
        mockPlaylists.find((p) => p.permalink === permalink) ||
        mockPlaylists[0];
      return { success: true, data: { ...found } };
    }
    const params = secretToken ? { token: secretToken } : {};
    const { data } = await pulsifyAxiosInstance.get(
      `/playlists/permalink/${permalink}`,
      { params },
    );
    return data;
  },
  ...
  async createPlaylist(payload) {
    ...
    const isPrivateVal =
      payload.is_private !== undefined ? payload.is_private : false;
  ...
  async updatePlaylist(playlistId, payload) {
      ...
      if (idx >= 0) {
        if (payload instanceof FormData) {
          if (payload.has("title"))
            mockPlaylists[idx].title = payload.get("title");
          if (payload.has("description"))
            mockPlaylists[idx].description = payload.get("description");
          if (payload.has("is_private"))
            mockPlaylists[idx].is_private =
              payload.get("is_private") === "true";
          if (payload.has("file")) {
            mockPlaylists[idx].cover_url = URL.createObjectURL(
              payload.get("file"),
            );
          }
        } else {
    ...
    const { data } = await pulsifyAxiosInstance.patch(
      `/playlists/${playlistId}`,
      payload,
    );
    return data;
  ...
  async deletePlaylist(playlistId) {
    ...
    const { data } = await pulsifyAxiosInstance.delete(
      `/playlists/${playlistId}`,
    );
    return data;
  },
  ...
  async addTrackToPlaylist(playlistId, trackId) {
    ...
    const { data } = await pulsifyAxiosInstance.post(
    ...
    if (isMock()) {
      ...
      if (pl) {
        pl.tracks = pl.tracks.filter(
          (t) => (t.track_id._id || t.track_id) !== trackId,
        );
        pl.track_count = pl.tracks.length;
      }
      return {
  ...
  async moveTrack(playlistId, trackId, newPosition) {
    ...
    const { data } = await pulsifyAxiosInstance.patch(
      `/playlists/${playlistId}/tracks/${trackId}/move`,
  ...
  async regenerateToken(playlistId) {
    if (isMock()) {
      ...
      return {
        ...
        data: { playlistId, secret_token: newToken },
      };
    }
    const { data } = await pulsifyAxiosInstance.post(
      `/playlists/${playlistId}/regenerate-token`,
      {},
    );
    return data;
  },
  ...
  async searchPlaylists(query, page = 1, limit = 20) {
    if (isMock()) {
      const matches = mockPlaylists.filter((p) =>
        p.title.toLowerCase().includes(query.toLowerCase()),
      );
      return { success: true, count: matches.length, data: matches };
    }
    const { data } = await pulsifyAxiosInstance.get("/playlists/search", {
