...
export const PulsifyLibraryPlaylists = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (isMounted) setPlaylistsError(err.message || 'Failed to retrieve playlists.');
      } finally {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (isMounted) setAlbumsError(err.message || 'Failed to retrieve albums.');
      } finally {
  ...
  const renderContent = () => {
    if (activeTab === 'Playlists') {
      if (playlistsLoading) return <div style={{ color: '#999', padding: '40px 0' }}>Loading playlists...</div>;
      if (playlistsError) return <div style={{ color: '#f50', padding: '40px 0' }}>{playlistsError}</div>;
      return (
        <>
    ...
    if (activeTab === 'Albums') {
      if (albumsLoading) return <div style={{ color: '#999', padding: '40px 0' }}>Loading albums...</div>;
      if (albumsError) return <div style={{ color: '#f50', padding: '40px 0' }}>{albumsError}</div>;
      return (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
            <h2 style={{ color: '#fff', fontSize: '18px', margin: 0 }}>Your albums and EPs:</h2>
            <div style={{ display: 'flex', gap: '12px' }}>
              <input
                type="text"
                placeholder="Filter"
          ...
          {albums.length === 0 ? (
            <div style={{ color: '#999', fontSize: '14px', marginTop: '20px' }}>
              ...
              <Link to="/upload" style={{ color: '#f50', textDecoration: 'none' }}>Upload multiple tracks</Link> to create one.
            </div>
          ) : (
            <div style={{
              display: 'grid',
              ...
              {albums.map((album) => {
                ...
                const artworkUrl = album.artwork_url || 'https://placehold.co/200x200/1a1a1a/333?text=💿';
                const creatorName = album.artist_id?.display_name || album.artist_id?.username || 'You';
                const trackCount = album.track_count || album.tracks?.length || 0;
                const albumType = album.type || 'Album';
                return (
                  <div key={albId} style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '8px', cursor: 'pointer' }}>
                    <Link to={`/albums/${albId}`} style={{ textDecoration: 'none' }}>
                      <div style={{ width: '100%', aspectRatio: '1/1', position: 'relative', overflow: 'hidden', borderRadius: '4px', backgroundColor: '#222' }}>
                        <img
                          src={artworkUrl}
                          alt={album.title}
                          style={{ width: '100%', height: '100%', objectFit: 'cover' }}
                          onError={(e) => { e.currentTarget.src = 'https://placehold.co/200x200/1a1a1a/333?text=💿'; }}
                        />
                        {/* Type badge */}
                        <div style={{
                        ...
                        <div style={{
                          ...
                          backgroundColor: 'rgba(0,0,0,0.7)', color: '#ccc',
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                      ...
                      <span
                        style={{ color: '#999', fontSize: '12px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginTop: '2px' }}
                      >
                        {creatorName}
                      </span>
                    </div>
          ...
          {TABS.map(tab => (
            <div
              ...
              style={{
                ...
                borderBottom: tab === activeTab ? '3px solid #f50' : '3px solid transparent',
                cursor: 'pointer',
                transition: 'color 0.15s'
              }}
              onMouseEnter={(e) => { if (tab !== activeTab) e.currentTarget.style.color = '#ccc'; }}
              onMouseLeave={(e) => { if (tab !== activeTab) e.currentTarget.style.color = '#999'; }}
            >
              {tab}
            </div>
          ))}
