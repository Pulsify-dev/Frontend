...
export const PulsifyPlaylistDetailView = () => {
    ...
    const fetchTarget = async () => {
      try {
        ...
        if (/^[a-fA-F0-9]{24}$/.test(playlistId)) {
          data = await PulsifyPlaylistService.getPlaylistById(
            playlistId,
            secretToken,
          );
        } else {
          data = await PulsifyPlaylistService.getByPermalink(
            playlistId,
            secretToken,
          );
        }
        ...
        if (
          ...
          (data.playlist._id || data.playlist.title)
        ) {
          data = data.playlist;
        ...
        try {
          const creatorId =
            data.creator_id?._id || data.creator_id || data.creator_username;
          if (creatorId) {
            const userRes = await pulsifyAxiosInstance.get(
              `/users/${creatorId}`,
            );
        ...
        const errorMsg =
          ...
          err.message ||
          "Error occurred fetching this set.";
  ...
  const handleCopyLink = () => {
    ...
    const identifier =
      playlistDetail.permalink || playlistDetail._id || playlistDetail.id;
  ...
  const handleRemoveTrack = async (indexToRemove) => {
    ...
    const trackId =
      trackEntry.track_id?._id || trackEntry.track_id || trackEntry.id;
    const newTracks = [...playlistDetail.tracks];
    ...
    try {
      await PulsifyPlaylistService.removeTrackFromPlaylist(
        playlistDetail._id,
        trackId,
      );
    } catch (err) {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      const res = await PulsifyPlaylistService.updatePlaylist(
        playlistDetail._id,
        formData,
      );
  ...
  const handleTogglePrivacy = async () => {
    ...
    try {
      await PulsifyPlaylistService.togglePrivacy(
        playlistDetail._id,
        newPrivacy,
      );
    } catch (err) {
  ...
  const handleOfflineDownload = () => {
    if (subscriptionTier !== "GO_PLUS") {
      alert(
        "Offline Listening is a Go+ Perk. Please upgrade to Go+ to download sets.",
      );
      navigate("/premium");
      return;
    }
  ...
  if (fetchError)
    return (
      <div style={{ ...loadingStyle, color: "#f44" }}>Error: {fetchError}</div>
    );
  if (!playlistDetail)
    return <div style={loadingStyle}>Playlist not found.</div>;

  const trackCount =
    playlistDetail.track_count || playlistDetail.tracks?.length || 0;
  const totalSec =
    playlistDetail.tracks?.reduce((s, t) => {
      ...
      <div style={{ maxWidth: "1240px", margin: "0 auto" }}>
        ...
        >
          ...
          >
            ...
            >
              ...
              <div style={{ minWidth: 0 }}>
                ...
                >
                  {playlistDetail.creator_id?.display_name ||
                    playlistDetail.creator_username ||
                    "You"}
                </span>
              </div>
              <span
              ...
              {(() => {
                ...
                >
                  ...
                  {waveformBars.map((h, i) => {
                    const isPlayed =
                      (i / waveformBars.length) * 100 <= (playerProgress || 0);
                    return (
                      <div
                        key={i}
                        style={{
          ...
          >
            <img
              src={
                playlistDetail.cover_url ||
                "https://placehold.co/340x340/2a1e30/666?text=♫"
              }
            ...
            >
              <button
                onClick={() =>
                  document.getElementById("detail-cover-upload").click()
                }
        ...
        >
          <div style={{ width: "120px", flexShrink: 0, textAlign: "center" }}>
            <div
              ...
              onClick={() => {
                const creatorId =
                  playlistDetail.creator_id?._id ||
                  playlistDetail.creator_id ||
                  playlistDetail.creator_username;
                if (creatorId) navigate(`/profile/${creatorId}`);
              }}
            >
              {creatorProfile?.avatar_url ||
              playlistDetail.creator_id?.avatar_url ? (
                <img
                  src={
                    creatorProfile?.avatar_url ||
                    playlistDetail.creator_id?.avatar_url
            ...
            <div
              style={{ fontSize: "13px", color: "#ccc", marginBottom: "2px" }}
            >
            ...
            >
              ...
              >
                <path
                  d="M5.75 2v12h1.5V2h-1.5zM13.25 10V6h-1.5v4h1.5zM4.25 5v6h-1.5V5h1.5zM8.75 4v8h1.5V4h-1.5z"
                  fill="currentColor"
                ></path>
              </svg>
              {trackCount}
          ...
          <div style={{ flex: 1, borderLeft: "none", paddingLeft: "0" }}>
            ...
            {playlistDetail.tracks && playlistDetail.tracks.length > 0 ? (
              ...
              <div
                style={{ color: "#555", padding: "30px 0", fontSize: "14px" }}
              >
          ...
          <div style={{ width: "300px", flexShrink: 0, marginTop: "-48px" }}>
            ...
            >
              ...
              <Link
                to={`/profile/${playlistDetail.creator_id?._id || playlistDetail.creator_id || playlistDetail.creator_username || ""}`}
                style={{
                  fontSize: "11px",
                  color: "#999",
                  cursor: "pointer",
            ...
            >
              {userPlaylists.slice(0, 3).map((pl, idx) => (
                ...
                >
                  <img
                    src={pl.cover_url || "https://placehold.co/50x50"}
                    alt={pl.title}
                    style={{
                      width: "50px",
                      height: "50px",
            ...
            <div style={{ fontSize: "11px", color: "#999", lineHeight: "1.6" }}>
              ...
              <br />
              <span style={{ color: "#60b8ff", cursor: "pointer" }}>
                Language: English (US)
              </span>
            </div>
          </div>
