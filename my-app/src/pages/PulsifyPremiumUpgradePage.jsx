...
export const PulsifyPremiumUpgradePage = () => {
  ...
  const handleUpgrade = async () => {
    try {
      ...
      setSuccessMessage('🎉 Payment successful! You are now subscribed to Artist Pro.');
    } catch (err) {
  ...
  const handleCancel = async () => {
    if (!window.confirm('Are you sure you want to cancel your Artist Pro subscription? You will lose access to unlimited uploads and premium features.')) {
      return;
    }
    try {
      setCancelling(true);
      ...
      setSubInfo(prev => ({
        ...prev,
        subscription: { ...(prev?.subscription || {}), plan: 'Free', status: 'Cancelled', cancel_at_period_end: false },
        effective_plan: 'Free',
        plan_limits: { plan: 'Free', can_upload: true, upload_track_limit: 10, album_limit: 2, album_track_limit: 5, is_ad_free: false, can_offline_listen: false },
      }));
      setUsageInfo(prev => ({
        ...prev,
        plan: 'Free',
        status: 'Cancelled',
        usage: {
          uploaded_tracks: { used: prev?.usage?.uploaded_tracks?.used ?? 0, limit: 10, remaining: Math.max(0, 10 - (prev?.usage?.uploaded_tracks?.used ?? 0)) },
  ...
  const trackLimit = usageInfo?.usage?.uploaded_tracks?.limit ?? (isPro ? null : 10);
  const trackRemaining = usageInfo?.usage?.uploaded_tracks?.remaining ?? (isPro ? null : Math.max(0, 10 - trackUsed));
  const usagePercent = trackLimit ? Math.min((trackUsed / trackLimit) * 100, 100) : 0;
    ...
    <div className="pulsify-premium-page">
          ...
          <div className="pulsify-usage-bar-hero">
            ...
            <div className="usage-bar-track">
              <div
                ...
                style={{
                  width: `${usagePercent}%`,
                  background: trackUsed > trackLimit
      ...
      {checkoutStep && (
        <div className="pulsify-checkout-progress">
          <div className={`checkout-step ${checkoutStep === 'checkout' ? 'active' : checkoutStep === 'webhook' || checkoutStep === 'done' ? 'done' : ''}`}>
            <div className="step-circle">1</div>
            <span>Creating checkout</span>
          </div>
          <div className="checkout-connector" />
          <div className={`checkout-step ${checkoutStep === 'webhook' ? 'active' : checkoutStep === 'done' ? 'done' : ''}`}>
            <div className="step-circle">2</div>
            <span>Processing payment</span>
          </div>
          <div className="checkout-connector" />
          <div className={`checkout-step ${checkoutStep === 'done' ? 'active done' : ''}`}>
      ...
      <div className="pulsify-pricing-section">
        <div className="pulsify-pricing-grid">
          ...
          {/* FREE TIER CARD */}
          <div className={`pulsify-pricing-card ${!isPro ? 'pulsify-pricing-current' : ''}`}>
            {!isPro && <div className="pulsify-current-badge">CURRENT PLAN</div>}
            ...
            {isPro ? (
              ...
              <button className="pulsify-btn-subscribe pulsify-btn-outline" disabled>
                Your current plan
              </button>
            )}
            ...
            <ul className="pulsify-perk-list">
              ...
              <li className="pulsify-perk-disabled">
                <svg className="pulsify-perk-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
          ...
          {/* ARTIST PRO CARD */}
          <div className={`pulsify-pricing-card pulsify-pricing-featured ${isPro ? 'pulsify-pricing-current' : ''}`}>
            {isPro ? (
              <div className="pulsify-current-badge pulsify-current-badge-pro">ACTIVE ★</div>
            ) : (
            ...
            {isPro ? (
              <button className="pulsify-btn-subscribe pulsify-btn-gold" disabled>
                ★ Your current plan
              </button>
            ) : (
              <button
            ...
            <ul className="pulsify-perk-list">
              <li>
                <svg className="pulsify-perk-icon pulsify-perk-icon-pro" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 2v20M2 12h20"/></svg>
                <span>Unlimited track uploads</span>
                <span className="pulsify-perk-badge pulsify-badge-unlimited">UNLIMITED</span>
              </li>
              <li>
                <svg className="pulsify-perk-icon pulsify-perk-icon-pro" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>
                <span>Unlimited albums</span>
                <span className="pulsify-perk-badge pulsify-badge-unlimited">UNLIMITED</span>
              </li>
              <li>
                <svg className="pulsify-perk-icon pulsify-perk-icon-pro" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
                <span>Advanced audience stats</span>
              </li>
              <li>
                <svg className="pulsify-perk-icon pulsify-perk-icon-pro" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
                <span>Ad-free experience</span>
                <span className="pulsify-perk-badge pulsify-badge-pro">PRO</span>
              </li>
              <li>
                <svg className="pulsify-perk-icon pulsify-perk-icon-pro" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
