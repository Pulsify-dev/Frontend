...
const getTrackId = (track) => track?.id ?? track?.trackId ?? track?.track_id ?? ''
...
const normalizeTrackInput = (track) => {
  ...
  return {
    ...
    duration: track.duration ?? track.durationSeconds ?? track.duration_seconds ?? 0,
...
const buildFallbackStreamInfo = (track, playbackContext = 'track_page') => {
  ...
  const discoveryLikeContext = contextKey === 'discovery' || contextKey === 'feed'
  const previewDurationSeconds =
    normalizedTrack?.previewDurationSeconds ??
    (discoveryLikeContext ? 30 : 0)
...
export const PlayerProvider = ({ children }) => {
  ...
  const hasPrevious =
    currentTrackIndex >= 0 && queueTrackIds.length > 1
  const hasNext =
    currentTrackIndex >= 0 && queueTrackIds.length > 1
  ...
  const loadTrack = useCallback(async (trackInput, options = {}) => {
    if (!trackInput) return null
    ...
    const nextStreamInfo = await resolveStreamInfo(resolvedTrack, nextPlaybackContext)
    if (loadRequestRef.current !== nextRequestId) {
      return resolvedTrack
    }

    const nextPlaybackState =
      nextStreamInfo?.playback_state ?? resolvedTrack.playbackState ?? 'Playable'
    ...
    const initialTime =
      typeof options.startTime === 'number'
        ...
        : nextPlaybackState === 'Preview' && nextPreviewDuration
          ? nextStreamInfo?.preview_start_seconds ?? resolvedTrack.previewStartSeconds ?? 0
          : 0
  ...
  const togglePlay = useCallback(async (trackInput = null, options = {}) => {
    ...
    const resumeTime =
      nextPlaybackState === 'Preview' && previewDurationSeconds
        ? currentTime >= previewEndSeconds || currentTime < previewStartSeconds
          ? previewStartSeconds
          : currentTime
        : currentTime
  ...
  const seekTo = useCallback((nextValue) => {
    if (!audioRef.current) return
    ...
    const targetTime =
      playbackState === 'Preview' && previewDurationSeconds
        ? Math.min(Math.max(nextValue, previewStartSeconds), previewEndSeconds)
        : nextValue
  ...
  const playRelativeTrack = useCallback(async (direction) => {
    if (!queueTrackIds.length || currentTrackIndex < 0) return

    const nextIndex =
      direction === 'previous'
        ? (currentTrackIndex - 1 + queueTrackIds.length) % queueTrackIds.length
